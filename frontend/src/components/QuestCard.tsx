    ...
    <Card className="hover:shadow-pixel-glow transition-all group relative overflow-hidden">
      ...
      <div className="absolute top-2 right-2 z-10">
        <div className={cn(
          ...
          <span className="font-pixel text-[0.4rem]">{difficulty.toUpperCase()}</span>
        </div>
      </div>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <CardTitle className="text-foreground leading-relaxed text-[0.6rem]">{title}</CardTitle>
          </div>
        </div>
      </CardHeader>
      
      <CardContent className="space-y-3">
        <p className="font-pixel-body text-pixel-base text-muted-foreground line-clamp-2">
          {description}
        </p>
        
        {/* Location */}
        <div className="flex items-center gap-2 p-2 bg-muted/50 border-2 border-border">
          <MapPin className="w-4 h-4 text-forest-light" strokeWidth={3} />
          <span className="font-pixel-body text-pixel-sm flex-1 truncate">{locationFrom}</span>
          <ChevronRight className="w-4 h-4 text-muted-foreground" strokeWidth={3} />
          <span className="font-pixel-body text-pixel-sm flex-1 truncate">{locationTo}</span>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1 px-2 py-1 bg-primary/20 border-2 border-primary/50">
            ...
            <span className="font-pixel text-[0.5rem] text-primary">RM {price.toFixed(2)}</span>
          </div>
          <div className="flex items-center gap-1 px-2 py-1 bg-xp/20 border-2 border-xp/50">
            <Zap className="w-4 h-4 text-xp" strokeWidth={3} />
            <span className="font-pixel text-[0.5rem] text-xp">+{xp} XP</span>
          </div>
          <div className="flex items-center gap-1 text-muted-foreground ml-auto">
            <Clock className="w-3 h-3" strokeWidth={3} />
            <span className="font-pixel text-[0.4rem]">{deadline}</span>
          </div>
        </div>
      </CardContent>
