...
// Helper function to extract AuthData from response (handles both array and single object)
const extractAuthData = (result: AuthData | AuthData[]): AuthData => {
  const authData =
...
export const questApi = {
  ...
  update: async (id: string, data: Partial<Quest>): Promise<Quest> => {
    const response = await apiClient.put<ApiResponse<Quest>>(
      `/quests/${id}`,
      data
    );
    return handleResponse<Quest>(response);
